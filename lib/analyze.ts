...
시각화 diagram은 해당 원문의 개념적 관계를 실제로 설명해야 합니다. 시간·작업 흐름은 flow, 반복 구조는 cycle, 계층은 layers로 선택하고 3~6개의 짧은 노드를 만드세요. comparison은 의미 있는 비교 대상을 2~4개 정하고, 모든 row.values 길이가 columns 길이와 같아야 합니다. 비교할 수 없는 수치를 지어내지 마세요.
