...
import { providerEnvironment, hermesEnvironment } from "../lib/provider-env";
...
test("Provider subprocess gets OS paths but no application credentials or injected startup variables", () => {
  for (const environment of [providerEnvironment, hermesEnvironment]) {
    const result = environment({
      ...
      HERMES_KANBAN_TASK: "injected",
      HERMES_API_CALL_STALE_TIMEOUT: "0",
    });
    ...
    assert.equal(
      result.HERMES_API_CALL_STALE_TIMEOUT,
      environment === hermesEnvironment ? "600" : undefined,
    );
  }
});
test("Request body limit is enforced without Content-Length, including Korean UTF-8", async () => {
  const oversized = new NextRequest("http://localhost", {
    method: "POST",
    body: JSON.stringify({ text: "가".repeat(110000) }),
  });
  await assert.rejects(
    () => body(oversized),
    (e: unknown) => (e as { status: number }).status === 413,
  );
  const valid = new NextRequest("http://localhost", {
    method: "POST",
    body: JSON.stringify({ text: "정상 입력" }),
  });
      ...
      () =>
        authenticate(
          new NextRequest("http://localhost", {
            headers: { Authorization: "Bearer fake-unit-test-token" },
          }),
        ),
